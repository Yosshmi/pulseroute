...
func ValidateURL(raw string, allowPrivate bool) error {
	...
	if u.Scheme != "https" && !(allowPrivate && u.Scheme == "http") {
		return fmt.Errorf("destination requires HTTPS")
	}
	if len(raw) > 2048 {
		return fmt.Errorf("destination URL too long")
	}
	return nil
}
func PublicIP(ip net.IP) bool {
	...
	for _, s := range []string{"100.64.0.0/10", "192.0.0.0/24", "192.0.2.0/24", "198.18.0.0/15", "198.51.100.0/24", "203.0.113.0/24", "240.0.0.0/4", "2001:db8::/32", "64:ff9b::/96"} {
	...
	tr.DialContext = func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		for _, ip := range ips {
			if !allowPrivate && !PublicIP(ip) {
				return nil, fmt.Errorf("destination address blocked")
			}
		}
